...
1. Jeżeli korzystasz na zajęciach ze swojego sprzętu to zainstaluj niezbędne oprogramowanie (uprzednio sprawdzając czy może już nie masz).
...
1. **Zainstaluj Python**: 
   - Sprawdź, czy Python jest zainstalowany na twoim komputerze. Możesz to zrobić, uruchamiając polecenie w terminalu:
     ...
     python --version
     ```
   - Jeśli nie masz Pythona zainstalowanego, pobierz go ze strony [oficjalnej Pythona](https://www.python.org/downloads/) i zainstaluj. Możesz też pobrać odpowiednią wersję pythona z Microsoft Store.
...
## Zadanie 2: Stworzenie Środowiska Wirtualnego (opcjonalnie)
Jeżeli używamy tej wersji pythona do większej ilości projektów lepiej użyć będzie środowiska wirtualnego, aby odiszolować potrzebne pakiety python do projektu.
...
1. **Stwórz nowy projekt Django**:
   - W terminalu, upewnij się, że jesteś w katalogu głównym swojego projektu. Następnie użyj polecenia:
     ...
     ```
     zamieniając [myproject] na nazwę swojego projektu
...
2. **Stwórz aplikację Django**:
   - W folderze projektu utwórz nową aplikację:
     ...
     ```
     zamieniając [myapp] na nazwę Twojej aplikacji.

  **Rozróżnienie**: Projekt Django to kontener dla aplikacji, podczas gdy aplikacja to konkretna część projektu, która wykonuje określone zadania (np. obsługa użytkowników, bloga itp.). Można też traktować jeden projekt jako całość, wtedy projekt będzie kontenerem dla ustawień django i całęgo frameworka, a aplikacja będzie kontenerem dla kodu i ustawień dla aplikacji. Z początku rozróżnienie tego może sprawiać problemy.
...
## !!!Wykonaj ten krok tylko jeżeli znasz już narzędzie GIT!!! Zadanie 5: Konfiguracja Repozytorium Git
...
3. **Wybór silnika bazy danych**:
   - Rozważ wybór silnika bazy danych (np. SQLite, PostgreSQL, MySQL) w zależności od wymagań projektu - preferowany SQLite3, który będzie dość wygodny do zastosowań projektu studenckiego.